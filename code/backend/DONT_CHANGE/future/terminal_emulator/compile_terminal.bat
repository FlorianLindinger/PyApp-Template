...
if exist "GIT_IGNORED\build" (
    echo Cleaning previous build in "GIT_IGNORED\build"...
    rmdir /s /q "GIT_IGNORED\build"
)
...
set "CMD=!CMD! --output-dir=GIT_IGNORED\build"
...
call !CMD!
if errorlevel 1 (
  echo Build failed. Press any key to exit. If it complains about missing modules, try to delete GIT_IGNORED folder in: 
  echo "%CD%"
...
set "SOURCE=GIT_IGNORED\build\%EXE_NAME%.dist"
if not exist "%SOURCE%" (
  for /f "delims=" %%D in ('dir "GIT_IGNORED\build\*.dist" /b 2^>nul') do (
    set "SOURCE=GIT_IGNORED\build\%%D"
    goto :found_dist
  )
  echo Dist folder not found: GIT_IGNORED\build\*.dist. Press any key to exit.
  pause > nul
  exit /b 1
)
:found_dist
...
if /i "%DO_PRUNE%"=="yes" (
    ...
    set "TARGET=GIT_IGNORED\build\pruned.dist"
