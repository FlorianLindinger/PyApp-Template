...
REM GIT_IGNORED\build, depending on DO_PRUNE.
set "FINAL_BUILD=GIT_IGNORED\build\pruned.dist"
if not exist "!FINAL_BUILD!\" (
    set "FINAL_BUILD=GIT_IGNORED\build\run.dist"
)
if not exist "!FINAL_BUILD!\" (
    for /f "delims=" %%D in ('dir "GIT_IGNORED\build\*.dist" /b 2^>nul') do (
        set "FINAL_BUILD=GIT_IGNORED\build\%%D"
        goto :found_dist
    )
)
...
:found_dist
if not exist "!FINAL_BUILD!\" (
    echo Final build folder not found in GIT_IGNORED\build.
...
set "BACKUP_DIR=GIT_IGNORED\compiled_backup"
