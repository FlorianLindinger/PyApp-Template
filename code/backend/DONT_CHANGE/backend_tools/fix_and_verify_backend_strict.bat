:: Description: Applies Ruff fixes, then runs strict backend verification.
::
:: ===========================
...
:run
cls
"%python_exe%" "%target_script%" strict --fix %* %run_options%
set "exit_code=%ERRORLEVEL%"
if not "%exit_code%"=="0" echo [Error] Backend strict verification failed with exit code %exit_code%.
set "run_options="
